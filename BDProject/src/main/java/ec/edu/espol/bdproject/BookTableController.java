...
private void setHandlers() {
    ...
    private void updateBook() {
        ...
        String query = "UPDATE Libro SET "
                + "titulo=?,descripcion=?,stock=?,categoria=?,autor=?,editorial=?,idioma=?,precio=?"
        ...
        try(PreparedStatement statement = Conexion.getInstance().conectar().prepareStatement(query)) {
            ...
            statement.setString(5, txtAutor.getText());
            statement.setString(6, txtEditorial.getText());
            statement.setString(7, txtIdioma.getText());
            statement.setFloat(8, Float.parseFloat(txtPrecio.getText()));
